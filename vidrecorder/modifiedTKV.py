...
class Player(Tk.Frame):
    ...
    def __init__(self, parent,controlParent, high, wide, title=None, video=''):
        #Tk.Frame.__init__(self, parent)

        self.parent = parent  # == root
        self.title = title
        #self.parent.title(self.title or "Workstation")
        ...
        self.videopanel.place(x=0,y=0, height=self.h,width=self.w)


        # self.playButton = ttk.Button(self.controlParent, text="Play", command=self.OnPlay)
        # stop            = ttk.Button(self.controlParent, text="Stop", command=self.OnStop)
        # self.muteButton = ttk.Button(self.controlParent, text="Mute", command=self.OnMute)
        # self.playButton.place(x=0,y=0,width=65,height=40)
        # stop.place(x=65,y=0,width=65,height=40)
        # self.muteButton.place(x=1135,y=0,width=65,height=40)

        # self.volMuted = False
        # self.volVar = Tk.IntVar()
        # self.volSlider = Tk.Scale(self.controlParent, variable=self.volVar, command=self.OnVolume,
        #                           from_=0, to=100, orient=Tk.HORIZONTAL,
        #                           showvalue=0, label = 'Volume')
        # self.volSlider.place(x=935,y=0,width=200,height=40)
        ...
        # self.timeSlider = Tk.Scale(self.controlParent, variable=self.timeVar, command=self.OnTime,
        #                            from_=0, to=1000, orient=Tk.HORIZONTAL,
        #                            showvalue=0)  # label='Time',
        # self.timeSlider.place(x=130,y=0,width=805,height=40)
        # self.timeSliderUpdate = time.time()
        # #timers.pack(side=Tk.BOTTOM, fill=Tk.X)

        self.playButton = ttk.Button()
        self.stopButton = ttk.Button()
        self.muteButton = ttk.Button()
        #self.playButton.place(x=0,y=0,width=65,height=40)
        #stopButton.place(x=65,y=0,width=65,height=40)
        #self.muteButton.place(x=1135,y=0,width=65,height=40)
    ...
    def OnStop(self, *unused):
        ...
        if self.player:
            ...
            #TODO: use timeSlider to set time based on slider object
            self._stopped = True
...
if __name__ == "__main__":
    ...
    root1 = Tk.Tk()
    player1 = Player(root1, video=_video)
    root1.protocol("WM_DELETE_WINDOW", player1.OnClose)  # XXX unnecessary (on macOS)
    root1.mainloop()
